...
def enabled_platforms(build_type_dir):
        '''Return a list of target names we know how to build.

        Strangely enough, this method used to live in build-tools!
        '''
        return ['safari', 'osx', 'firefox', 'reload', 'web', 'chrome', 'ios',
                'android', 'wp', 'ie', 'ios-native', 'android-native', 'crosswalk']
...
class Build(object):
	...
	def __init__(self, config, source_dir, output_dir,
		...
		if config is not None and "android_sdk_root" in config:
			config["android_sdk_dir"] = os.path.join(config['android_sdk_root'], '24.0.2')
		...
		if system_config is not None and "android_sdk_root" in system_config:
			system_config["android_sdk_dir"] = os.path.join(system_config['android_sdk_root'], '24.0.2')
