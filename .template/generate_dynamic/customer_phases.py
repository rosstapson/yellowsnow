...
def include_name(build):
	...
	return [
		...
		{'when': {'platform_is': 'crosswalk'}, 'do': {'set_attribute_value_xml': {
			"file": 'development/crosswalk/AndroidManifest.xml',
			"element": "application",
			"attribute": "android:label",
			"value": xml_safe_name
		}}},
...
def include_uuid():
	return [
		...
		{'when': {'platform_is': 'crosswalk'}, 'do': {'find_and_replace': {
			"in": ('development/crosswalk/AndroidManifest.xml',),
			"find": "io.trigger.forge.android.inspector", "replace": "${core.android.package_name}"
		}}},
		{'when': {'platform_is': 'wp'}, 'do': {'find_and_replace': {
